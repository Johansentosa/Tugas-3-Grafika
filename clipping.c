...
Point pointClipLeftUp, pointClipRightDown;
int moveFactor = 20; //pergeseran clip
...
void drawLineX(Point* p1, Point* p2, Color* c, int positif) {
    ...
    for (i = p1 -> x; i <= p2 -> x; i++) {
    	if (!kaboom || i>=pointClipLeftUp.x && i<=pointClipRightDown.x && j>=pointClipLeftUp.y && j<=pointClipRightDown.y) {
...
void drawPeopleWithParachute(Point initialPosition) {
	Point temp;
	Color faceColor; setColor(&faceColor, 255, 255, 0);
...
void * peopleFall(void * p22){
	Point * p = (Point *) p22;
	Point temp; setPoint(&temp,p->x,p->y+200);
	while (temp.y < (vinfo.yres - 200)) {
    ...
    while (!kaboom) { // selama pesawat belum ketembak
        while (headPlane > 0) { // selama pesawat belum mentok di kiri
            if (kaboom) { // pesawat kena tembak
                ...
                drawClip(pointClipLeftUp, pointClipRightDown, cDestroy);
                ...
                for(j = 0; j < 4; j++) {
                    drawLine(&sayap[j], &sayap[j + 1], &bg);
                }
               // drawCircle(12, circle, 2, bg);
                drawLine(&sayap[5], &sayap[0], &bg);
...
int main() {
    setColor(&bg, 0, 0, 255);
    setColor(&cBorder, 255, 255, 0);
   	setPoint(&pointClipLeftUp, 100, 300);
    setPoint(&pointClipRightDown, 400, 600);
    connectBuffer();
    clearScreen(&bg);
    drawClip(pointClipLeftUp, pointClipRightDown, cBorder);
    pthread_t thrPlane, thrLasergun, thrBeam, thrFallWheel, thrFallWheel2, thrPeopleFall, thrClip;
